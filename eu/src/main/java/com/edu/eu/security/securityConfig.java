...
public class securityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		...
		http.csrf(cus -> cus.disable())
				.authorizeHttpRequests(aut -> aut.requestMatchers(HttpMethod.POST, "/sede-save").hasAnyRole("ADMINS","USERS")
					//.requestMatchers(HttpMethod.POST,"/dato-sav").hasAnyRole("USERS", "ADMINS")
					.requestMatchers(HttpMethod.GET, "/sede-all").authenticated()
					.requestMatchers(HttpMethod.PUT, "/sede-update").hasAnyRole("ADMINS","USERS")
					.requestMatchers(HttpMethod.GET, "/sede-id").hasAnyRole("ADMINS", "USERS")
